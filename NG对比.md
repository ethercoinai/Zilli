# hermes-ng vs hermes-web-ui 对比分析

| 维度 | **hermes-web-ui** (现有) | **hermes-ng** (本项目) |
|------|------------------------|----------------------|
...
2. **抽象层级不同** — `hermes-web-ui` 是 Agent 的用户界面；`hermes-ng` 是 Agent 的训练工厂
3. **数据流方向不同** — web-ui 消费 Agent 输出；hermes-ng 生产 Agent（通过 RL 训练）
4. **可能的衔接点**：hermes-ng 训练的模型可部署到 Hermes Agent，通过 web-ui 展示；`ContinuousLearner` 的设计意图是从生产环境（含 web-ui 产生的交互数据）吸收轨迹反哺训练
...
用户交互层  ──  hermes-web-ui (Vue/Koa 面板)
                   ...
                   Hermes Agent
                      ▲ 部署
模型训练层  ──  hermes-ng (Python RL 框架)
```

web-ui 是 "用户与 Agent 交互的界面层"，hermes-ng 是 "构建 Agent 的训练基础设施层"。
